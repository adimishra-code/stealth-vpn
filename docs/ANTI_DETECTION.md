...
**Platform coverage — enforced by default (no manual toggle needed):**

| Platform | Kill switch | How it is enforced |
|---|---|---|
| Linux (wg-quick) | Enforced | `PostUp`/`PreDown` iptables REJECT rules (config key `BlockUntunneledTraffic` is ignored by wg-quick) |
| Android (official app) | Enforced | Generated config includes `BlockUntunneledTraffic = true` — honored by the app automatically |
| iOS (official app) | Enforced | `BlockUntunneledTraffic = true` — honored by the app automatically |
| macOS (official app) | Enforced | `BlockUntunneledTraffic = true` — honored by the app automatically |
| Windows (official app) | Enforced | `BlockUntunneledTraffic = true` — honored by the app automatically |

The `.conf` generated by the platform carries `BlockUntunneledTraffic = true`
(a key honored by the official WireGuard apps on Android/iOS/macOS/Windows)
together with the `PostUp`/`PreDown` rules for wg-quick on Linux — so the
kill switch is on for every platform out of the box, with nothing for the
user to toggle. Users who import the config into a third-party client should
verify that client supports the key (most do not) and enable the client's
own kill-switch option instead.
